...
- `PARTS`: object keyed by category id → array of part objects. Every part has `id`,
  `name`, `price` (euros, integer), `spec` (short tagline). Optional cross-cutting fields:
  `img` (product-photo URL, https only — else line-art), `legacy: true` (previous-gen /
  cycled-out → "Prev-gen" badge + availability warning on the detail page + report note),
  and `perf` on **cpu & gpu** (relative gaming index, RTX 5090 / R7 9800X3D = 100) which
  drives `estimatePerformance()` in the summary. Plus rich, **accurate
...
- **2026-07-01 — legacy parts + real photos + buyer guidance + perf estimate (`v=13`):** owner
  asked to (a) bring RTX 40 back for people who still want them, (b) warn on cycled-out parts,
  (c) put real recommendations + use-cases on product pages, (d) add "a fancy little performance
  estimate" by the price, and (e) use the product images they supplied. Done:
  • Restored 12 previous-gen GPUs (RTX 40/30, RX 7000/6000) + marked AM4/Alder-Lake CPUs, AM4
    boards and DDR4 as `legacy: true` → "Prev-gen" card badge, an amber availability warning on
    the detail page, and a report note.
  • Wired the owner image set (see task 3) — `img` on ~39 parts, `partThumb()` photo + line-art
    fallback, `.has-photo` light plate.
  • Added `partAdvice(cat, part)` → an **"Our take"** detail block (tier chip + recommendation +
    "best for" tags), generated by heuristics over the real spec fields (no hand-written blurbs).
  • Added `perf` (gaming index) to every CPU/GPU and `estimatePerformance(build)` → a
    performance card inside the price card (tier, ≈avg FPS, target resolution, a 0–100 bar, and
    a note; GPU-dominated with a weak-CPU cap). Verified: Sweet preset → High-end / ≈122 FPS /
    1440p Ultra; 4090 page shows the Prev-gen warning + Flagship "Our take". Assets bumped `v=13`.
...
3. [DONE — PHOTOS RE-ADDED FROM OWNER SET] **Part imagery = real product photo when we have
   one, bespoke line-art otherwise.** History: a first photo experiment (5 mismatched Wikimedia
   marketing shots) was removed 2026-07-01 for looking janky, and the catalog went line-art
   only. Then the owner **supplied a consistent image set** (`hardware_product_images.xlsx`,
   175 white-background Newegg/brand-CDN product shots) and asked for photos back. Now:
   `partThumb()` renders `p.img` (a hosted URL) with an `onerror` fallback to `partArt()`; the
   media plate gets `.has-photo` (a light gradient so white-bg product shots blend) and reverts
   to the dark line-art plate if the image 404s. ~39 parts have a matched `img`; the rest
   (incl. all current-gen RTX 50 / RX 9000, which the sheet had no shots for) stay on line-art.
   The mix reads as deliberate (real product = light plate, illustrated = dark blueprint). The
   sandbox **does** reach the Newegg/ASUS CDNs, so photos are verifiable in preview (mind
   `loading="lazy"` — below-fold ones show `naturalWidth 0` until scrolled). Only wire
   **https** URLs (one Palit `http://` shot was dropped — mixed-content-blocked on Pages).
